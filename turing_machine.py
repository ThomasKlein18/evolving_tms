...
class TuringMachine :
    ...
    def __init__(self, other, matr, num_states, alphabet_size, mutation_rate):
        """
        TuringMachine Constructor
        Setting up the band, filling the delta function with random values, choosing end states randomly

        tm = a TM for copying. Set to None if you want to create a random TM
        matr = the input matrix
        ...
        if other is None :
            # setting up band and read/write head
            self.band = matr.flatten().astype(np.int32)
            self.head = 0

            # setting up delta function
            self.alphabet = TuringMachine._count_unique_colours(matr) + 5
            ...
            self.end_states = np.unique(np.random.choice(np.arange(1,num_states), 3))
    ...
    def step(self) :
        ...
        # call delta function
        self.state, new_symbol, new_move = self.delta[self.state, current_symbol, :]
    ...
    def procreate(self) :
        ...
        """
        alan = TuringMachine(self, 0, 0, 0, 0)
        alan._mutate()
        return alan
    ...
    def _mutate(self) :
        ...
        for _ in range(self.mutation_rate) :
            ...
            if rval > 0.66 :
                ...
                self.delta[x,y,0] = int(np.floor(np.random.rand() * self.num_states))

            if rval < 0.66 and rval > 0.33 :
                # change one write operation
                self.delta[x,y,1] = int(np.floor(np.random.rand() * self.alphabet))
